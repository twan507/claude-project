...
- Rule 1 (no fabrication, nguồn cho mọi claim): system prompt mục 4.1
- Rule 2 (tin tức bắt buộc song song DB + web): `agent_db_00` mục 2
- Rule 3 (biệt danh, thuật ngữ lạ phải hỏi trước): `agent_db_00` mục 4.1
- Rule 4 (clarification trước câu phức tạp): system prompt mục 4.4 + `agent_db_00` mục 4.2
- Rule 5 (không bịa số, xác suất, phân bổ phải có giả định): `agent_db_00` mục 4.3
- Rule 6 (K hygiene, không lộ ký hiệu raw và taxonomy): system prompt mục 4.5 + `agent_db_00` mục 5
- Rule 7 (rollback sạch khi sai giả định gốc): system prompt mục 4.3
