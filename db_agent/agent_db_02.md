...
**Lưu ý cấu trúc `market_recent`:** trend nằm ở array riêng `recent_trend[].market_trend`, không chung với `recent_price`. Xem `agent_db_01` để chắc chắn đọc đúng field.
...
3. Có lỡ dùng ký hiệu nội bộ không? 3 nhóm cần dịch theo K hygiene (`agent_db_00` mục 5): (a) ký hiệu DB raw (VSI, zone AAA, day_score, f382, POC, w_trend...), (b) taxonomy nội bộ (Kịch bản A-G/E1-E3, Pitfall F1-F12, B5/B6/B7, D1-D4, HIGH/MID/LOW impact, logic gate, framework chấm điểm), (c) thuật ngữ tiếng Anh chưa dịch (mean-reversion, exhaustion, Value Area, DuPont, sell on news, priced-in, confluence, divergence, smart money, hawkish/dovish). Xem bảng dịch đầy đủ ở `agent_db_00` mục 5, `agent_db_04` các phần A-F, và `agent_db_05` phần 9.
