# agent_db_00 — Master file

## 1. Mục đích & scope

Knowledge base cho phân tích chứng khoán Việt Nam dựa trên MongoDB `agent_db`. Bộ tài liệu này chứa schema, query patterns, anti-patterns, methodology diễn giải chỉ báo, và methodology phân tích tin tức.
...
- Không hỗ trợ tư vấn cho retail/client cuối — bộ tài liệu này giả định audience là analyst/broker nội bộ (xem mục 4.4)
...
**MongoDB `agent_db`** là nguồn CHÍNH cho số liệu định lượng (giá, dòng tiền, BCTC, kỹ thuật, vĩ mô từ `other_data`, tin tức trong DB). Quyền read-only. Schema chi tiết ở `agent_db_01`, pipeline mẫu ở `agent_db_02`.
...
Khi cần clarify trước câu hỏi phân tích (theo rule system prompt mục 4.4), domain này dùng 2 câu hỏi chuẩn:
...
Bộ tài liệu này giả định user là analyst/broker nội bộ, được phép nhận khuyến nghị cụ thể. Nếu project chuyển cho audience khác (retail, client cuối, intern chưa chứng chỉ), cần thay bằng knowledge base phù hợp — bộ này không phục vụ được vì constraint nội dung rộng hơn mức cho phép retail.
...
Danh sách 18 mã ngắn (user nhập) ↔ tên DB chuẩn (`industry_name` / `industry`) + cách áp dụng filter (cả Default + Override mode) và xử lý re-rank: xem **`agent_db_01`** đầu Section B "Khối ngành".
...
Rule K hygiene ở system prompt mục 4.5 bắt buộc dịch ký hiệu raw và taxonomy nội bộ sang ngôn ngữ tự nhiên. Bộ tài liệu này định nghĩa 3 nhóm cần dịch và bảng dịch tương ứng.
...
Thuật ngữ tiếng Anh trong phân tích tin tức (bảng dịch đầy đủ ở `agent_db_05` phần 9): "sell on news", "priced-in", "dot plot", "forward guidance", "hawkish/dovish", "contango/backwardation", "tailwinds", "confluence", "divergence", "smart money", "wash-out", "risk-off/risk-on", "pump and dump", "going concern", "cross-default", "one-off", "pre-sales".
...
| Rank ngành-vs-ngành | DB không lưu — tự tổng hợp sort `week_score` (dòng tiền tuần) qua 18 ngành whitelist; xem `agent_db_01` mục "Xếp hạng ngành" |
...
## 10. Output rules khi trả lời user

Mọi output tới user phải đảm bảo:

- Số liệu định lượng đã quy đổi đơn vị theo mục 6
- K hygiene: ký hiệu DB raw, taxonomy nội bộ, thuật ngữ tiếng Anh chưa phổ thông đều đã dịch theo bảng mục 5 (và `agent_db_05` phần 9 cho news terminology)
- Mỗi claim định lượng có nguồn truy được, không lộ tên collection hay field DB: mô tả tự nhiên "theo dữ liệu [loại X] trong hệ thống" cho số từ DB, URL hoặc tên báo cho nguồn web
- Thông tin vĩ mô tháng (CPI, PMI, XNK) hoặc tin tức có ghi chú ngày cập nhật
- Biệt danh thị trường không chuẩn đã được xác nhận với user hoặc xác minh qua web search trước khi đưa vào phân tích (mục 4.1)
