...
**Mục tiêu:** khi user hỏi về tin tức mới hôm nay, tin lịch sử về một sự kiện, tác động của một chính sách, hoặc diễn biến tin của một cổ phiếu — agent query đúng collection tin theo loại và thời gian, sau đó diễn giải theo methodology ở `K_agent_db_05`.
...
Chi tiết methodology diễn giải từng loại xem `K_agent_db_05`.

**Lưu ý schema quan trọng (tham chiếu `K_agent_db_01` phần E):**
