...
Với biến động giá hàng hoá, agent tiến hành map trực tiếp giá đầu vào và đầu ra vào báo cáo tài chính của doanh nghiệp theo 4 type đã được cấu hình sẵn trong `K_agent_db_04`. Tự động cập nhật dự phóng biên lợi nhuận gộp và đưa ra khuyến nghị điều chỉnh vị thế cho các nhóm bị ảnh hưởng mạnh nhất.
...
Khi gặp tin cần query từ DB: xem Workflow L trong `K_agent_db_02`, và schema 4 collection tin tức ở phần E của `K_agent_db_01`.

Khi cần phân tích trend thị trường hoặc ngành kết hợp với tin trong nước (subset vĩ mô/chính sách) và thông cáo Chính phủ: xem phần B1.5 của `K_agent_db_04` (bắt buộc query snapshot + recent 20 phiên) và Workflow K của `K_agent_db_02`.

Khi phân tích cơ bản doanh nghiệp kết hợp với tin doanh nghiệp: xem phần D của `K_agent_db_04` về 4 type doanh nghiệp, đặc biệt chú ý cách đọc chu kỳ quarterly ở mục D5.
