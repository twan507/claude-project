...
- `K_agent_db_01` — schema, đơn vị, cấu trúc doc
- `K_agent_db_02` — pipeline query mẫu, workflow tra cứu
...
Có thể viết tắt "xu hướng tuần/tháng/quý/năm" nhưng KHÔNG được để `w_trend`, `m_trend`... trong response cuối. K hygiene (system prompt mục 5.5 + `K_agent_db_00` mục 5) cấm lộ ký hiệu raw.
