...
**Trigger:** User yêu cầu "viết báo cáo tuần", "weekly market report", "báo cáo thị trường tuần", "tổng hợp thị trường tuần", "chiến lược tuần tới"; hoặc đầu tuần (sáng thứ Hai) khi user gửi context để chạy báo cáo.

**Depends:** `K_agent_db`.

**Status:** Active.

### P_recommendation_memo

**Mục đích:** Sinh báo cáo khuyến nghị mua mã đơn lẻ phục vụ gửi trực tiếp cho khách hàng. Chỉ khuyến nghị MUA (long only), horizon 1-6 tháng. Báo cáo có 15 mục theo template VBSE-like (cover → tóm tắt → foundation → 4-7 luận điểm flex → execution → bear case → kịch bản → disclaimer). MD là source of truth, render pptx khi user yêu cầu.
...
Pack độc lập với `P_invest_memo` — user gọi lúc nào cũng được. Nếu user có memo tier 5C của mã, agent đọc và base trên thesis sẵn có. Không có thì build fresh từ DB. **Không khuyến nghị BÁN/GIỮ/short/derivative.** Không dùng chỉ báo trend.

**Master:** `P_recommendation_memo_00`

**Trigger:** User yêu cầu "viết khuyến nghị [TICKER]", "recommendation [TICKER]", "báo cáo MUA [TICKER]", "báo cáo phân tích [TICKER] gửi khách hàng", "khuyến nghị mã [TICKER]".
...
### O_recommendation_memo

**Mục đích:** Render spec cho deliverable của pack `P_recommendation_memo` — báo cáo khuyến nghị mã đơn lẻ 15 mục với structure rigid theo template VBSE-like. MD là source of truth, render pptx 15 slide khi user yêu cầu (format chính cho gửi KH), docx khi user muốn archive formal. Quy định layout pptx mỗi slide với visual element bắt buộc (stat callout, icon, bảng), color palette theo branding, K hygiene, mapping disclaimer footer.

Khi P pack abort ở checkpoint 1 hoặc 2, O pack KHÔNG render file — tránh sản phẩm half-baked có thể bị gửi nhầm cho KH.

**Master:** `O_recommendation_memo_00`

**Trigger:** Activate cùng với `P_recommendation_memo` khi user yêu cầu báo cáo khuyến nghị mã.

**Depends:** `P_recommendation_memo`, `K_agent_db`.
...
**Mục đích:** Visual template branded VBSE cho render báo cáo pptx. Pack cung cấp 27 layout cover toàn bộ pipeline báo cáo VBSE: 5 cover (recommendation / deepdive / weekly market / macro-sector / generic) + 1 section divider + 12 content layout (bullet, stat callout grid, big stat subpoints, two-column info, comparison 3 kịch bản, stat+table session, data table 8x8, timeline, risk steelman 6-cell, variant perception, buy structure flex, take profit targets) + 5 layout extension (thesis-B panel right, heatmap 24 ngành, peer compare table, mini-charts 2x2, calendar tuần) + 3 chart placeholder layout (line full width + scatter peer + donut composition) + 1 disclaimer.

Template dùng pattern placeholder `{{NAME}}` upper_snake_case + slide name = LAYOUT_ID. AI khi render: clone slide theo LAYOUT_ID phù hợp → fill placeholder text giữ font/color/alignment → tại chart placeholder build native PowerPoint chart từ YAML block trong MD source (chart strategy decoupled với T — T chỉ giữ vị trí + kích thước placeholder).

Color palette navy `#003D7A` + đỏ `#D71249` + xanh `#059669` + Calibri. Signature pattern: vertical red bar `0.08" × 0.70"` cạnh trái title + tam giác vuông cân đỏ decorative ở 4 vị trí chuẩn (`bottom_right`, `bottom_left`, `top_right`, `left_edge_navy`).

Nội dung lặp lại fix cứng (không placeholder): tên phòng ban "PHÒNG MÔI GIỚI VÀ TƯ VẤN ĐẦU TƯ", tổ "Tổ tầm soát cổ phiếu", website "www.vbse.vn", hotline "1900 588 866", disclaimer body 3 đoạn pháp lý chuẩn, các tiêu đề báo cáo cố định ("BÁO CÁO PHÂN TÍCH" / "PHÂN TÍCH CHUYÊN SÂU" / "BÁO CÁO THỊ TRƯỜNG TUẦN" / "TUYÊN BỐ MIỄN TRỪ TRÁCH NHIỆM"). Bỏ author name placeholder.

Pack có 14 quy tắc bắt buộc cho AI render (layout ID immutable, placeholder format upper_snake_case, reuse layout có sẵn không tự tạo mới, chart placeholder strategy, không hardcode hex color, format số locale vi-VN, constraint kịch bản không gán xác suất, honest steelman, variant perception 3 sub bắt buộc, heatmap override màu theo magnitude, big stat constraint ≤ 6-8 ký tự, tam giác signature 4 vị trí chuẩn, self-check trước save, file source of truth không tự sửa template).

**Master:** `T_vbse_00`

**Trigger:** Activate cùng với O pack khi tất cả điều kiện đúng: (a) user yêu cầu deliverable pptx (không phải MD/docx), (b) audience là VBSE (gửi KH VBSE / họp nội bộ VBSE / branding VBSE), (c) P pack workflow đã hoàn thành sinh MD source. Không activate khi: user chỉ cần MD/docx, hoặc audience là brand khác (Finext, internal khác chưa có T pack), hoặc workflow chưa sinh MD.

**Depends:** Không depends vào O cụ thể nào — pack độc lập với O, có thể dùng với mọi O pack có MD output. Mapping layout → loại báo cáo (recommendation memo / weekly market / invest memo deep-dive) trong file `_00` chỉ là reference, agent runtime quyết.
...
T_{brand_or_purpose}_{NN}    ví dụ T_vbse_00, T_finext_00, T_internal_00
```

Số `NN` ý nghĩa nội bộ pack, quy định trong file `_00` của pack đó. Riêng T pack thường có convention: `_00` master MD (placeholder schema, layout list, mapping, quy tắc render), `_01+` file binary (pptx/docx/xlsx) hoặc skeleton code.
